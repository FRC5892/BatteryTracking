...
public class NTApi {
    ...
    public NTApi(String teamOrIP) {
        ...
        this.logPub = table.getStructArrayTopic("log", BatteryTracking.Battery.LogEntry.struct).publish();
    }
    public void loop() {
        ...
        while(true) {
            ...
            if (!this.hasRead) {
                ...
                if (insertedBattery != null) {
                    ...
                    this.logPub.set(insertedBattery.getLog().toArray(new BatteryTracking.Battery.LogEntry[0]));
                }
            }
            if (this.triggerWriteSub.get()) {
                BatteryTracking.updateSync(this.usageSupplierAH.getAsDouble());
